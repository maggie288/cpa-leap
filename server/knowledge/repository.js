...
export const dedupeAutoGeneratedEntries = ({
  subject,
  topicKeyword = 'AI生成知识条目',
  topicExact = '',
  keepLatest = 1,
  actor = 'system:dedupe',
} = {}) => {
  ...
  const rows = base.filter((row) => {
    ...
    if (topicExact) return String(row.topic || '') === topicExact
    return String(row.topic || '').includes(topicKeyword)
  })
  if (rows.length <= keepLatest) {
    return { changedCount: 0, deprecatedIds: [], totalCandidates: rows.length }
  }

  const ts = (row) =>
    dayjs(String(row.updatedAt || row.reviewedAt || row.createdAt || '')).isValid()
      ? dayjs(String(row.updatedAt || row.reviewedAt || row.createdAt || '')).valueOf()
      : 0

  const sorted = [...rows].sort((a, b) => ts(b) - ts(a))
  const deprecatedTargets = sorted.slice(Math.max(0, keepLatest))
  const targetSet = new Set(deprecatedTargets.map((row) => row.id))
  const deprecatedIds = []

  for (let i = 0; i < base.length; i += 1) {
    if (!targetSet.has(base[i].id)) continue
    base[i] = {
      ...base[i],
      status: 'deprecated',
      reviewedBy: actor,
      reviewedAt: now,
      updatedAt: now,
      version: Number(base[i].version || 1) + 1,
    }
    deprecatedIds.push(base[i].id)
  }

  if (deprecatedIds.length) writeKb(base)
  return { changedCount: deprecatedIds.length, deprecatedIds, totalCandidates: rows.length }
}
